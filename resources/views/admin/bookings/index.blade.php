        ...
        <div class="p-4 sm:p-6">
            ...
            <form method="GET" action="{{ route('admin.bookings.index') }}" id="monthly-filter" class="filter-form">
                <div class="flex flex-wrap gap-4 items-end">
                    ...
                    <div class="flex-1 min-w-[150px]">
                        <label for="status_monthly" class="block text-sm font-medium text-gray-700 mb-1">Status</label>
                        <select name="status" id="status_monthly" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
            ...
            <form method="GET" action="{{ route('admin.bookings.index') }}" id="daterange-filter" class="filter-form hidden">
                <div class="flex flex-wrap gap-4 items-end">
                    ...
                    <div class="flex-1 min-w-[150px]">
                        <label for="status_daterange" class="block text-sm font-medium text-gray-700 mb-1">Status</label>
                        <select name="status" id="status_daterange" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                            <option value="">Semua Status</option>
                            <option value="pending" {{ request('status') == 'pending' ? 'selected' : '' }}>Pending</option>
                            <option value="approved" {{ request('status') == 'approved' ? 'selected' : '' }}>Disetujui</option>
                            <option value="rejected" {{ request('status') == 'rejected' ? 'selected' : '' }}>Ditolak</option>
            ...
            <div class="mt-4 pt-4 border-t border-gray-200">
                <div class="flex flex-wrap gap-2">
                    ...
                    @if(request('month') && request('year'))
                        <a href="{{ route('admin.bookings.download-report', array_filter(['month' => request('month'), 'year' => request('year'), 'status' => request('status')])) }}" 
                    ...
                    @if(request('start_date') || request('end_date'))
                        <a href="{{ route('admin.bookings.download-report', array_filter(['start_date' => request('start_date'), 'end_date' => request('end_date'), 'status' => request('status')])) }}" 
                    ...
                    <!-- Download Hanya Berdasarkan Status -->
                    @if(request('status') && !request('month') && !request('year') && !request('start_date') && !request('end_date'))
                        <a href="{{ route('admin.bookings.download-report', ['status' => request('status')]) }}" 
                    ...
                    <div class="hidden md:block overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                ...
                                @empty
                                <tr>
                                    <td colspan="8" class="px-6 py-4 text-center text-sm text-gray-500">Tidak ada booking</td>
                                </tr>
                                @endforelse
                            </tbody>
                        </table>
